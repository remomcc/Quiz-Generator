...
class QuizGenerator:
    ...
    def generate_question_with_vectorstore(self):
        ...
        if self.llm is not None:
            st.success("Successfully initialized LLM!", icon="✅")

        else:
            self.llm = self.init_llm()
        
        if self.vectorstore:
            st.success("Successfully initialized vectorstore!", icon="✅")
        else:
            st.error("Vectorstore is not initialized on the class! Please specify vectorstore argument.", icon="🚨")
